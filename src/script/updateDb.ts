...
const updateFeeds = async (feeds: FeedUpdate[]) => {
	...
	for (const feed of feeds) {
		const hash = await transformBlurhash(feed.image);
...
const insertFeeds = async (feeds: FeedUpdate[]) => {
	...
	for await (const feed of feeds) {
		...
		if (data.categories) {
			...
			const hash = await transformBlurhash(data.image);
	...
	while (batch < categories.length) {
		...
		const podcast = podcasts.filter((item) => cat.some((tag) => item.tags?.includes(tag)));
